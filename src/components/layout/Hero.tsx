  ...
  const titleVariants = {
    ...
    visible: {
      transition: {
        ...
        staggerChildren: reduceMotion ? 0 : 0.026,
      },
    },
  };
  ...
  const letterVariants = {
    hidden: reduceMotion
      ...
      : {
          ...
          textShadow: "0 0 18px rgba(52, 211, 153, 0.65)",
        },
    visible: {
      opacity: 1,
      y: 0,
      ...
      <div className="relative flex min-h-[420px] h-[60vh] w-full flex-col items-center justify-center overflow-hidden px-4 pb-8 text-center shadow-lg sm:h-[65vh] sm:px-6 md:h-[75vh] md:justify-end md:pb-12">
        ...
        >
          ...
          <motion.h1
            ...
            initial="hidden"
            animate="visible"
            aria-label="Tudo que seu Pet precisa, com a entrega que ele merece!"
            className="px-2 text-2xl font-black leading-tight tracking-tight text-white sm:text-3xl md:text-5xl"
          >
            {HERO_TITLE_LINES.map((line, lineIndex) => (
              <span
                key={line}
                aria-hidden="true"
                className={lineIndex === 1 ? "block md:mt-1" : "block"}
              >
